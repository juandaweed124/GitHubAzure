...
export class ReporteCuantitativoComponent implements OnInit {
  ...
  graficoTres() {
    console.log(this.datosClienteTable[0]);
      ...
      options: {
        animation: {
          onComplete: () => {
            ...
            console.log(this.imageZscoreUno );
          }
        },
        plugins: {
          legend: {
      ...
      options: {
        animation: {
          onComplete: () => {
            ...
            console.log(this.imageZscoreDos );
          }
        },
        plugins: {
          legend: {
  ...
  async generarPdf(FlagConsolidado: boolean = false) {
    ...
    this.reporteCuantitativoService.getDatos(json).subscribe(async res => {
      ...
      let newListAnalytes = this.datosFinRonda.filter(x => x.Serialsample === muestraSeleccionda);
      newListAnalytes = newListAnalytes.map(x => x.Analyte);
      
    for (const key in newListAnalytes) {
      let consolidado = newListAnalytes[key] !== newListAnalytes[newListAnalytes.length - 1] ? true : false;
      if (newListAnalytes.length === 1) consolidado = false;
      
      this.buscarAnalitos(newListAnalytes[key], muestraSeleccionda, consolidado);
      ...
      await new Promise((res, e) => {
        setTimeout(() => {
          ...
          const nuevaTablaEstadistica = this.tablaEstadisticaGeneral.map((x,i) => {
            return {
              ...
              B_Media: x[0].media,
              C_DS: x[0].DS,
              D_CV: (x[0].CV * 100).toFixed(1) + "%",
              E_UM: x[0].Um,
              F_grupo:grupos[i]
            }
          })

          const nuevaTablaZscore = this.datosClienteTableDivZscore
            .filter(y => y.Serialsample === muestraSeleccionda)
            .map(x1 => {
              let iguales = this.jsonResumenMuestra.DataAnalytes.filter(z => z.IdAnalytes === -1)[0]?.statisticalGeneral;
              if(!iguales){
                iguales = this.jsonResumenMuestra.DataAnalytes.filter(p => p.IdAnalytes === x1.idAnalytes)[0].statisticalGeneral;    
              }
              return {
                A_resultado: x1.Resultado,
                B_zScore: x1.Zscore,
                C_media: x1.Media,
                D_ds: x1.DS,
                E_Grupo: iguales
          ...
          const caberosTablasZscore = ['Resultado Ronda', 'Z-score', 'Media', 'DS','Grupo'];
          ...
          const caberosTablasResumenRonda = ['Ronda', 'Muestra', 'Analito', 'Resultado', 'Media','DS', 'CV', 'UM', 'Z-Score'];
        ...
        if (newListAnalytes[key] === newListAnalytes[newListAnalytes.length - 1]) {
          setTimeout(() => {
            let clienteInfo:any={};
            if (this.flagCliente) {
              clienteInfo = {
